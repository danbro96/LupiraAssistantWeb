...
/// that echoes back what the proxy actually sent (path + auth + forwarded-prefix headers), so tests can assert
/// the pass-through behavior instead of trusting the transform code.
/// </summary>
public sealed class StubUpstream : IAsyncDisposable
{
    ...
    public static async Task<StubUpstream> StartAsync()
    {
        ...
        app.Map("/{**path}", (HttpContext ctx) => Results.Json(new UpstreamEcho(
            ...
            ctx.Request.Headers["X-Forwarded-Prefix"].ToString())));
...
public sealed record UpstreamEcho(string Path, string Authorization, string XDevUser, string XForwardedPrefix);
