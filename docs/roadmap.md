...
- **Candidate-selection over free generation** — the `Resolve × RefKind` profile family (backbone: Agent run profiles). **`Resolve × Contact` (batch 6) + `Resolve × Event` (batch 7) shipped** — deterministic-first ladder → LLM candidate-pick on ambiguity, `ActionRetargeted` for create-vs-update, now behind a reusable `IProposalResolver` + per-kind `IResolveStrategy` seam. Remaining: `Resolve × Task/Place/Container` (same shape; each needs its candidate-fetch client method — and Task additionally a new `UpdateTask` write path) and career refs (with the career wiring).
...
- Two-phase `Scan → Extract` + `RunProfile` spine + `ParentRunId` shipped (batch 5) as the only extraction path. `Resolve × Contact` (batch 6) + `Resolve × Event` (batch 7) shipped behind a reusable `IProposalResolver`/`IResolveStrategy` seam; the rest of the `Resolve × RefKind` family (Task/Place/Container/Career) remains unbuilt.
...
2. **Extend the `Resolve × RefKind` family** — `Resolve × Contact` (batch 6) + `Resolve × Event` (batch 7) shipped behind the `IProposalResolver`/`IResolveStrategy` seam; apply the same ladder to Task (dedup + update-vs-create — additionally needs a new `UpdateTask` write path), then Place/Container, then career refs (with the career wiring). Each needs its candidate-fetch client method — the run profile, ladder, and `ActionRetargeted` plumbing are reusable. Separately, the comms Participant→Contact write-back (endpoint + `senderContactId` payload field + `contactId` search filter) rides on `Resolve × Contact`.
