...
- **Candidate-selection over free generation** — the `Resolve × RefKind` profile family (backbone: Agent run profiles). **`Resolve × Contact` shipped (batch 6)** — deterministic-first ladder → LLM candidate-pick on ambiguity, `ActionRetargeted` for create-vs-update. Remaining: `Resolve × Event/Task/Place/Container` (same shape; each needs its candidate-fetch client method) and career refs (with the career wiring).
...
- Two-phase `Scan → Extract` + `RunProfile` spine + `ParentRunId` shipped (batch 5) as the only extraction path. `Resolve × Contact` shipped (batch 6); the rest of the `Resolve × RefKind` family (Event/Task/Place/Container/Career) remains unbuilt.
...
2. **Extend the `Resolve × RefKind` family** — `Resolve × Contact` shipped (batch 6); apply the same ladder to Event/Task (dedup + update-vs-create), then Place/Container, then career refs (with the career wiring). Each needs its candidate-fetch client method (event/task search) — the run profile, ladder, and `ActionRetargeted` plumbing are reusable. Separately, the comms Participant→Contact write-back (endpoint + `senderContactId` payload field + `contactId` search filter) rides on `Resolve × Contact`.
